...
| 유형 | 등록 방법 | 비고 |
|---|---|---|
| Ollama 로컬 모델 | `ORCHESTRATOR_MODEL`, `WORKER_MODEL` 환경변수 | 컨테이너 시작 시 자동 다운로드 |
...
워크로드를 재실행하면 모든 Ollama 모델이 재다운로드된다. 이를 피하고 싶을 때는 컨테이너 터미널에서 직접 추가한다.

```bash
# 1. 모델 다운로드
ollama pull ollama/qwen3:14b

# 2. .env에 WORKER_MODEL 추가 (공백으로 구분)
#    예: 기존 "ollama/gemma4:31b" → "ollama/gemma4:31b ollama/qwen3:14b"
nano /home/node/.openclaw/.env

# 3. 설정 재생성 + gateway reload
bash /usr/local/bin/reload.sh
```

봇(Telegram)에게 요청할 때는 `shell_execute` 도구를 사용해 위 절차를 수행하도록 지시할 수 있다. 상세 절차는 `skills/ollama-exec/SKILL.md` 참조.

#### /models 표시 제한 (Known Bug)

OpenClaw v2026.4.11 Issue [#65500](https://github.com/openclaw/openclaw/issues/65500): `WORKER_MODEL`에 등록된 모델이 `/models` 목록에 표시되지 않을 수 있다. `generate-config.sh`에서 `agents.defaults.model.fallbacks`에 등록하는 방식으로 워크어라운드를 적용했으나, upstream 패치 전까지는 표시 누락이 발생할 수 있다.

- **직접 전환**: `/model ollama/qwen3:14b` 명령으로 모델을 명시 지정하면 `/models` 목록 표시와 무관하게 사용 가능
- **세션 초기화**: `/new` 입력 시 `ORCHESTRATOR_MODEL`로 복귀
