...
라우팅 판단은 오케스트레이터가 자연어 정책을 해석하여 수행하므로, 트리거 조건이 명확하게 일치할수록 일관성이 향상됩니다.
...
파일 저장 트리거는 오케스트레이터가 결과의 크기와 형태를 사전에 판단하여 결정합니다. 결과 형태에 따라 일부 케이스에서 트리거가 인식되지 않을 수 있습니다.

상세 규칙은 `templates/AGENTS.md` 참조.

### 동작 신뢰도에 관한 안내

> 본 이미지의 모델 라우팅, 결과 전달, 컨펌 게이트 등 주요 동작은 `templates/AGENTS.md`에 정의된 자연어 지침을 오케스트레이터 모델이 해석·실행하는 방식으로 구현되었습니다. 결정론적 보장이 아닌 모델 기반 동작이므로, 사용된 모델의 instruction-following 특성과 작업의 복잡도에 따라 일부 규칙 미준수가 발생할 수 있습니다.
>
> 비용 또는 시스템 상태에 영향을 주는 작업(외부 API 호출, 파일 생성·수정·삭제)은 사용자 컨펌 단계를 거치도록 설계되어 있습니다. 운영 환경에서의 사용은 권장 모델 조합에서의 동작을 사전 검증한 후 진행하시기를 권장합니다.
