...
1. Dropbox 최상위 경로에 **`OpenClaw_Auth`** 폴더 생성 (보안을 위한 전용 폴더)
2. `storage_state.json` 파일을 `OpenClaw_Auth/` 폴더에 업로드

### [Step 3] gcube 워크로드 배포 시 설정

gcube 대시보드에서 워크로드 배포 시 **다음 두 가지를 반드시 모두 설정해야 합니다.**

| 항목 | 설정값 |
|---|---|
| 볼륨 마운트 | Dropbox `OpenClaw_Auth` 폴더 → 컨테이너 `/mnt/notebooklm` |
| 환경변수 | `NOTEBOOKLM_HOME=/mnt/notebooklm` |
