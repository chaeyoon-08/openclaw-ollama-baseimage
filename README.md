...
| `multi-agent` | `ghcr.io/chaeyoon-08/openclaw-bot-multi:multi-agent` | IPAI 멀티 에이전트 + 오케스트레이션 + NotebookLM MCP |
...
> 브랜치: `multi-agent` | 이미지: `ghcr.io/chaeyoon-08/openclaw-bot-multi:multi-agent`
...
# 3. 인증 파일 확인
# Linux/Mac: ~/.notebooklm/storage_state.json
...
| 폴더명 | 용도 | 비고 |
|---|---|---|
| `OpenClaw_Auth` | NotebookLM Google 인증 파일 보관 | `storage_state.json` 업로드 |
| `OpenClaw_Data` | 에이전트 상태 데이터 전체 보관 | 비워두면 됨 (컨테이너가 자동 초기화) |

`storage_state.json`을 `OpenClaw_Auth/` 폴더에 업로드합니다. `OpenClaw_Data/`는 지금은 비워둡니다.
...
| ③ | 환경변수 | — | `NOTEBOOKLM_HOME=/mnt/notebooklm` | NotebookLM 경로 지정 |
...
```bash
nlm login
# 생성된 storage_state.json을 Dropbox OpenClaw_Auth/ 폴더에 덮어쓰기
# 그 후 gcube 워크로드 재시작
```
...
| `NOTEBOOKLM_HOME` | `/mnt/notebooklm` | NotebookLM 인증 파일 디렉터리. Dropbox 볼륨 마운트 경로와 일치시킬 것 |
...
| Dropbox `OpenClaw_Auth` → `/mnt/notebooklm` | NotebookLM 인증 파일. `NOTEBOOKLM_HOME=/mnt/notebooklm` 환경변수와 함께 설정 |
