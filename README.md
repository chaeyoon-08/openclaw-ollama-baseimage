...
- `agents.defaults.subagents`: `maxSpawnDepth: 1`, `maxConcurrent: 4` — 서브 에이전트는 `ORCHESTRATOR_MODEL` 상속
...
| 유형 | 등록 방법 | 비고 |
|---|---|---|
| Ollama 로컬 모델 | `ORCHESTRATOR_MODEL` 환경변수 (오케스트레이터 1개 지정) | 컨테이너 시작 시 자동 다운로드 |
| Ollama 추가 모델 | gcube `Ollama_Models/` 클라우드 저장소 마운트 또는 봇 요청으로 런타임 추가 | `/models`에 자동 표시 |
| 외부 API 모델 | `MODEL_API_KEY` 환경변수 | Anthropic, OpenAI, Google 등 |

vllm, llama.cpp 등 Ollama 이외의 로컬 추론 엔진은 현재 미지원.

#### gcube 클라우드 저장소 마운트 (모델 영속화)

gcube `Ollama_Models/` 폴더를 `/root/.ollama`로 마운트하면 모델 파일이 영속 저장된다. 워크로드를 재시작해도 재다운로드 없이 즉시 기동된다.

| gcube 버킷 폴더 | 컨테이너 경로 |
|---|---|
| `Ollama_Models/` | `/root/.ollama` |

설정 방법: gcube 포털 → 워크로드 → Storage Management → 마운트 경로 `/root/.ollama` 지정
...
봇(Telegram)에게 요청할 때는 `shell_execute` 도구를 사용한다. 상세 절차는 `templates/TOOLS.md` 참조.
...
│     └── /root/.ollama              ← gcube Ollama_Models/ 마운트 (영속 저장)
├── openclaw gateway (0.0.0.0:18789)
│     ├── Main Agent (ORCHESTRATOR_MODEL)
│     │     └── sessions_spawn → Worker Agent (ORCHESTRATOR_MODEL 상속, maxDepth=1)
│     └── Telegram long-polling
