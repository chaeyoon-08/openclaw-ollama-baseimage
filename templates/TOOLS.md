...
(모델은 컨테이너 내부에 저장되며 워크로드 재시작 시 재다운로드됨)

**발동 조건:** 사용자가 새 Ollama 모델 설치 또는 등록 요청 시.

**절차:** `skills/ollama-exec/SKILL.md` 참조.
...
**상세 절차:** `skills/ollama-exec/SKILL.md` 참조

---

## 스킬: nlm-login

NotebookLM MCP 인증이 만료되었을 때 재인증하는 스킬.

**발동 조건:** NLM 도구 호출 시 인증 오류, 또는 사용자가 `/nlm_login` 입력.

**상세 절차:** `skills/nlm-login/SKILL.md` 참조.
